    ...
    if ($save_type == 'upload') {
        ...
        foreach ($options as $option_array) {
            ...
            if (isset($option_array['id'])) { // Non - Headings...
                ...
                if (is_array($type)) {
                ...
                } elseif ($new_value == 'false' && $type == 'checkbox') { // Checkbox Save
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if(isset($options_to_save[$multicheck_id]) && $options_to_save[$multicheck_id] !=''){
                    ...
                    if(isset($output[$option_array['id'] . '_size']) && $output[$option_array['id'] . '_size'] !=''){
                        $typography_array['size'] = $output[$option_array['id'] . '_size'];
                    }

                    if(isset($output[$option_array['id'] . '_face']) && $output[$option_array['id'] . '_face'] != '') {
                        $typography_array['face'] = stripslashes($output[$option_array['id'] . '_face']);
                    }

                    if(isset($output[$option_array['id'] . '_style']) && $output[$option_array['id'] . '_style'] != '') {
                        $typography_array['style'] = $output[$option_array['id'] . '_style'];
                    }

                    if(isset($output[$option_array['id'] . '_color']) && $output[$option_array['id'] . '_color'] != '') {
                        $typography_array['color'] = $output[$option_array['id'] . '_color'];
                    }

                    if(!empty($typography_array)) {
                    ...
                    if(isset($output[$option_array['id'] . '_width']) && $output[$option_array['id'] . '_width']!= '') {
